  ...
  return (
    <div key={item.id} className=" flex flex-col items-start p-4 border-b">
      <div className=" h-auto ">
        <a href="/" className=" font-bold text-lg leading-10">
      ...
      <div className=" leading-6">
        {selected ? (
          ...
          <a
            href="/"
            onClick={handleClick}
            className=" cursor-pointer hover:text-stone-600 text-stone-800 line-clamp-2"
          >
            {item?.target?.excerpt.substring(0, 80) + "..."}
            <span href="/" className=" text-base leading-7 text-blue-500">
              阅读全文 &gt;
            </span>
          </a>
        )}
      </div>
      <div className={'flex bg-white w-full ' + (selected ? 'sticky bottom-0 shadow-sm border-t ' : '')}>
        ...
        <div className=" font-semibold text-stone-500 p-2 m-2 inline-flex">
          ...
          <div className=" flex justify-center items-center h-6 w-6 pb-2">
            ...
          </div>
        </div>
        {
            selected && <div onClick={handleClick} className=" font-semibold text-stone-500 p-2 m-2 inline-flex cursor-pointer ">
  ...
  return (
    <div className="flex flex-col">
      {commandListData.map((item) => (
        <CommandData key={item.id} item={item} />
      ))}
    </div>
  );
}
